...
function ArriveResultTable({ rows }: { rows: UpdatedRow[] }) {
  const { widths, startResize } = useResizableColumns({ index: 40, customer: 160, unitArrive: 100, change: 80 })
  return (
    <table className="w-full text-sm" style={{ tableLayout: "fixed" }}>
      <thead>
        <tr className="border-b border-cream-border text-left">
          ...
          <th className="px-5 py-2 text-xs font-medium text-gray-500 text-right relative select-none" style={{ width: widths.unitArrive }}>
            Unit Arrive
